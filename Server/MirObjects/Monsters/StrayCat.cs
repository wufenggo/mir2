...
namespace Server.MirObjects.Monsters
{
    ...
    public class StrayCat : MonsterObject
    {
        ...
        protected override void Attack()
        {
            ...
            if (distance <= 1 && RandomUtils.Next(6) != 1)
            {
            ...
            for (int i = 1; i <= distance; i++)
            {
                ...
                if (target == Target.CurrentLocation)
                {
                    if (attacktype == 0)
                    {
                        ...
                        action = new DelayedAction(DelayedType.Damage, Envir.Time + delay+200, Target, damage * 3 / 2, DefenceType.ACAgility);
                        ActionList.Add(action);
                    }
                    else
                    {
                        action = new DelayedAction(DelayedType.Damage, Envir.Time + delay, Target, damage, DefenceType.ACAgility);
                        ActionList.Add(action);
                        action = new DelayedAction(DelayedType.Damage, Envir.Time + delay+ delay, Target, damage * 2, DefenceType.MACAgility);
                        ActionList.Add(action);
                    }
                }
                else
                    ...
                    for (int o = 0; o < CurrentMap.Objects[target.X, target.Y].Count; o++)
                    {
                        ...
                        if (ob.Race == ObjectType.Monster || ob.Race == ObjectType.Player)
                        {
                            ...
                            if (attacktype == 0)
                            {
                                ...
                                ActionList.Add(action);
                                action = new DelayedAction(DelayedType.Damage, Envir.Time + delay + 200, ob, damage * 3 / 2, DefenceType.ACAgility);
                                ActionList.Add(action);
                            }
                            else
                            {
                                action = new DelayedAction(DelayedType.Damage, Envir.Time + delay, ob, damage, DefenceType.ACAgility);
                                ActionList.Add(action);
                                action = new DelayedAction(DelayedType.Damage, Envir.Time + delay + delay, ob, damage * 2, DefenceType.MACAgility);
                                ActionList.Add(action);
                            }
                        }
                    }
