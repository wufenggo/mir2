    ...
    public sealed class PlayerObject : MapObject
    {
            ...
            else if (message.StartsWith("@"))
            {
                ...
                switch (parts[0].ToUpper())
                {
                    ...
                    case "RESTORE":
                        ...
                        MessageQueue.Enqueue(string.Format("玩家 {0} 已经被 {1} 恢复", data.Name, Name));
                        break;
       ...
       public void TalkMonster(uint objectID)
        ...
        public bool Match(AuctionInfo info)
        {
            ...
            return (UserMatch || !info.Expired && !info.Sold) && ((MatchType == ItemType.Nothing || info.Item.Info.Type == MatchType) && (info.Item.Info.Shape >= MinShapes && info.Item.Info.Shape <= MaxShapes) && (GradeType == ItemGrade.None || info.Item.Info.Grade == GradeType) &&
        ...
        public void GetMarket(string name, ItemType type, ItemGrade Grade)
        {
            Search.Clear();
        ...
        public void MarketSearch(string match, ItemType Type, ItemGrade Grade)
        {
            if (Dead || Envir.Time < SearchTime) return;
        ...
        public void MarketRefresh()
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                GetMarket(string.Empty, MatchType, GradeType);
            }
        }
        public void MarketBuy(ulong auctionID, bool MailItems)
        {
            ...
            for (int n = 0; n < CurrentMap.NPCs.Count; n++)
            {
                ...
                foreach (AuctionInfo auction in Envir.Auctions)
                {
                    ...
                    MarketSearch(MatchName, MatchType, GradeType);
                    return;
                }
            }
        ...
        public void GuildStorageGoldChange(Byte Type, uint Amount)
        {
            ...
            else
            {
                ...
                {
                    ReceiveChat("已达金币上限", ChatType.System);
                    return;
                }
                if (MyGuildRank.Index != 0)
                {
                    ReceiveChat("等级不足.", ChatType.System);
                    return;
                }
        ...
        public void GuildStorageItemChange(Byte Type, int from, int to)
        {
            ...
            {
                Enqueue(p);
                ReceiveChat("你不是公会的一员.", ChatType.System);
                return;
            }
            ...
            {
                Enqueue(p);
                ReceiveChat("你不能在安全区域外使用公会存储.", ChatType.System);
                return;
            }
            ...
            switch (Type)
            {
                case 0://store
                    ...
                    {
                        Enqueue(p);
                        ReceiveChat("你没有权限将物品储存在公会仓库中.", ChatType.System);
                        return;
                    }
                    if (from < 0 || from >= Info.Inventory.Length)
                    {
                    ...
                    {
                        ReceiveChat("目标插槽不为空.", ChatType.System);
                        Enqueue(p);
                        return;
                    }
        ...
        public void RefreshMount(bool refreshStats = true)
        {
            if (RidingMount)
            {
                ...
                {
                    RidingMount = false;
                    ReceiveChat("你需要一个马鞍", ChatType.System);
                }
                ...
                {
                    RidingMount = false;
                    ReceiveChat("没有缰绳你不能在这里骑马", ChatType.System);
                }
            }
            else
            {
        ...
        public void CollectRefine()
        {
            ...
            if (index == -1)
            {
                ReceiveChat(String.Format("你的背包没有足够的空间, 腾出一些空间再试一次", Info.CurrentRefine.FriendlyName), ChatType.System);
                Enqueue(p);
                return;
            }
