...
namespace Server.MirEnvir
{
    public class PublicEvent
    {
                ...
                for (int d = 0; d <= Info.EventSize; d++)
                {
                    for (int y = CurrentLocation.Y - d; y <= CurrentLocation.Y + d; y++)
                    {
                        ...
                        for (int x = CurrentLocation.X - d; x <= CurrentLocation.X + d; x += Math.Abs(y - CurrentLocation.Y) == d ? 1 : d * 2)
                        {
                            ...
                            if (!Map.Valid(x, y) || Map.Objects[x, y] == null) continue;

                            for (int i = 0; i < Map.Objects[x, y].Count; i++)
                            {
                                MapObject ob = Map.Objects[x, y][i];
                                switch (ob.Race)
                                {
        ...
        public void ActivateEvent()
        {
            ...
            MessageQueue.EnqueueDebugging(string.Format("地图{0} 激活事件:{1}", Map.Info.FileName, Info.EventName));
        ...
        private void SpawnInvasionMonsters(int order)
        {
            ...
            {
                MessageQueue.EnqueueDebugging(string.Format("取消激活入侵事件：{0}阶段：{1}", Info.EventName, Stage));
                IsActive = false;
            }
        ...
        private void SpawnMonstersForInvasion(List<EventRespawn> respawns)
        {
            ...
            foreach (var eventRespawn in respawns)
            {
                ...
                if (monster == null)
                {
                    MessageQueue.EnqueueDebugging(string.Format("{0}事件：{1}失败，找不到怪物{2}", Map.Info.FileName, Info.EventName, eventRespawn.MonsterName));
                    continue;
                }
        ...
        private void SpawnMonsters()
        {
            ...
            foreach (var eventRespawn in Info.Respawns)
            {
                ...
                if (monster == null)
                {
                    MessageQueue.EnqueueDebugging(string.Format("{0}激活事件：{1}失败，找不到怪物{2}", Map.Info.FileName, Info.EventName, eventRespawn.MonsterName));
                    return;
                }
        ...
        public void EventMonsterDied(List<PlayerObject> monsterContributers)
        {
            ...
            switch (Info.EventType)
            {
                ...
                default:
                    ...
                    {
                        MessageQueue.EnqueueDebugging(string.Format("取消激活事件:{0}", Info.EventName));
                        IsActive = false;
                    }
                    break;
            }
