...
public class PathFinder
{
    ...
    {
        //bmp = DrawFunctions.DrawFilledRectangle(Grid.GetLength(0), Grid.GetLength(1));
        ...
        SetPixel(startNode.Location, Color.Blue);
            ...
            foreach (Node neighbor in GetNeighbours(currentNode))
            {
                ...
                if (newMovementCostToNeighbor < neighbor.GCost || !openSet.Contains(neighbor))
                {
                    ...
                    if (!openSet.Contains(neighbor))
                    {
                        ...
                        SetPixel(neighbor.Location, Color.Blue);
                    }
                    else
                    {
                        openSet.UpdateItem(neighbor);
                        SetPixel(neighbor.Location, Color.Red);
                    }
                }
            }
        }
    ...
    public List<Node> RetracePath(Node startNode, Node endNode)
    {
        ...
        while (currentNode != startNode)
        {
            ...
            SetPixel(currentNode.Location, Color.Green);
        }

        path.Add(startNode);
        SetPixel(startNode.Location, Color.Green);
        path.Reverse();
    ...
    private void SetPixel(Point p, Color color)
    {
        if (bmp == null) return;
    ...
    {
        if (bmp == null) return;
        bmp.Save(new Random().Next().ToString() + ".png");
    }
    #endregion
}