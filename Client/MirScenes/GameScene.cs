...
namespace Client.MirScenes
{
    public sealed class GameScene : MirScene
    {
        ...
        private void GuildStorageGoldChange(S.GuildStorageGoldChange p)
        {
            ...
            {
                case 0:
                    ChatDialog.ReceiveChat(String.Format("{0}将{1}金币捐赠给公会基金.", p.Name, p.Amount), ChatType.Guild);
                    GuildDialog.Gold += p.Amount;
                    break;
                case 1:
                    ChatDialog.ReceiveChat(String.Format("{0}从公会基金中获得了{1}金币.", p.Name, p.Amount), ChatType.Guild);
        ...
        {
            MirMessageBox messageBox = new MirMessageBox(string.Format("{0} 要跟你结婚.", p.Name), MirMessageBoxButtons.YesNo);
            ...
            if (HoverItem.Info.Durability > 0)
            {
                switch (HoverItem.Info.Type)
                {
                    case ItemType.Amulet:
                        ...
                        break;
                    case ItemType.Ore:
                        text += string.Format("\n纯度: {0}", Math.Round(HoverItem.CurrentDura / 1000M));
                        break;
                    case ItemType.Meat:
                        text += string.Format("\n质量: {0}", Math.Round(HoverItem.CurrentDura / 1000M));
                        break;
                    case ItemType.Mount:
                        text += string.Format("\n忠诚: {0} / {1}", HoverItem.CurrentDura, HoverItem.MaxDura);
                        break;
                    case ItemType.Food:
                        text += string.Format("\n营养 {0}.", HoverItem.CurrentDura);
                        break;
                    case ItemType.Fish:
        ...
        public MirControl AttackInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (minValue != 0 || addValue != 0)
            {
                ...
                {
                    text = string.Format("经验率 + {0}% ", minValue);
                }
                else
                {
        ...
        public MirControl DefenceInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel HEALTH_RECOVERYLabel = new MirLabel
                {
                    ...
                    Text = string.Format(addValue > 0 ? "生命 恢复 + {0} (+{1})" : "生命 恢复 + {0}", minValue + addValue, addValue)
                };
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                count++;
                if (HoverItem.Info.Type != ItemType.Gem)
                    text = string.Format(addValue > 0 ? "毒抵抗 + {0} (+{1})" : "毒抵抗  + {0}", minValue + addValue, addValue);
                else
                    text = string.Format("增加{0}毒抵抗 ", minValue + maxValue + addValue);
        ...
        public MirControl WeightInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel HANDWEIGHTLabel = new MirLabel
                {
                    ...
                    Text = string.Format(addValue > 0 ? "手腕负重 + {0} (+{1})" : "手腕负重 + {0}", minValue + addValue, addValue)
                };
            ...
            if (minValue > 0 || maxValue > 0 || addValue > 0)
            {
                ...
                MirLabel WEARWEIGHTLabel = new MirLabel
                {
                    ...
                    Text = string.Format(addValue > 0 ? "持久 + {0} (+{1})" : "持久 + {0}", minValue + addValue, addValue)
                };
            ...
            if (HoverItem.Info.Type == ItemType.Potion && HoverItem.Info.Durability > 0)
            {
                ...
                MirLabel TNRLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("时效 : {0}", Functions.PrintTimeSpanFromSeconds(HoverItem.Info.Durability * 60))
                };
            ...
            if (HoverItem.Info.Type == ItemType.Transform && HoverItem.Info.Durability > 0)
            {
                ...
                MirLabel TNRLabel = new MirLabel
                {
                    ...
                    OutLine = true,
                    Parent = ItemLabel,
                    Text = string.Format("时效 : {0}", Functions.PrintTimeSpanFromSeconds(HoverItem.Info.Durability, false))
                };
        ...
        public MirControl NeedInfoLabel(UserItem item, bool Inspect = false)
        {
            ...
            #region CLASS
            if (realItem.RequiredClass != RequiredClass.通用)
            {
                count++;
                Color colour = Color.White;
    ...
    public sealed class MapControl : MirControl
    {
        ...
        public static bool AutoRun
        {
            ...
            set
            {
                ...
                _autoRun = value;
                if (GameScene.Scene != null)
                    GameScene.Scene.ChatDialog.ReceiveChat(value ? "[自动跑: On]" : "[自动跑: Off]", ChatType.Hint);
            }
    ...
    public class Buff
    {
        ...
        public override string ToString()
        {
            ...
            switch (Type)
            {
                //magic
                case BuffType.TemporalFlux:
                    text = string.Format("时间通量\n 通过: {0} MP 增加下一个传送的成本", (int)(MapObject.User.MaxMP * 0.3F));
                    break;
                case BuffType.Hiding:
                    text = "隐身\n对大多数怪物不可见.\n";
                    break;
                case BuffType.Haste:
                    ...
                    break;
                case BuffType.SwiftFeet:
                    text = string.Format("快速脚\n增加移动速度: {0}.\n", Values[0]);
                    break;
                case BuffType.Fury:
                    text = string.Format("血龙剑法\n增加攻击速度: {0}.\n", Values[0]);
                    break;
                case BuffType.LightBody:
                    text = string.Format("身轻如燕\n增加敏捷: {0}.\n", Values[0]);
                    break;
                case BuffType.SoulShield:
                    text = string.Format("魔法防御盾\n增加魔法防御: 0-{0}.\n", Values[0]);
                    break;
                case BuffType.BlessedArmour:
                ...
                case BuffType.MentalState:
                    ...
                    {
                        case 0:
                            text = string.Format("激进 (全部伤害)\n不能穿过障碍.\n", Values[0]);
                            break;
                        case 1:
                            text = string.Format("技巧射击 (轻量伤害)\n可以穿过障碍.\n", Values[0]);
                            break;
                        case 2:
                            text = string.Format("小组模式 (中等伤害)\n相信你的队友.\n", Values[0]);
                            break;
                    }
                    break;
                case BuffType.MagicBooster:
                ...
                case BuffType.BagWeight:
                    ...
                    break;
                case BuffType.Transform:
                    text = string.Format("变换\n 伪装你的外观。\n");
                    break;
                case BuffType.RelationshipEXP:
                    text = string.Format("恋爱天使\n经验增加 by: {0}%.\n", Values[0]);
                    break;
                case BuffType.Mentee:
                    text = string.Format("训练有素\n技能经验获取速度加倍\n经验增加: {0}%.\n", Values[0]);
                    break;
                case BuffType.Mentor:
                    text = string.Format("导师的教导\n伤害增加 {0}%.\n", Values[0]);
                    break;
                case BuffType.Guild:
                    text = string.Format("行会主管\n");
                ...
                //stats
                case BuffType.Impact:
                    text = string.Format("冲击力\n增加攻击: 0-{0}.\n", Values[0]);
                    break;
                case BuffType.Magic:
                    text = string.Format("魔法\n增加魔法: 0-{0}.\n", Values[0]);
                    break;
                case BuffType.Taoist:
                    text = string.Format("道士\n增加道术: 0-{0}.\n", Values[0]);
                    break;
                case BuffType.Storm:
                    text = string.Format("风暴\n增加攻击速度: {0}.\n", Values[0]);
                    break;
                case BuffType.HealthAid:
                    text = string.Format("保健箱 \n增加生命值: {0}.\n", Values[0]);
                    break;
                case BuffType.ManaAid:
                    text = string.Format("魔法援助\n增加魔法值: {0}.\n", Values[0]);
                    break;
                case BuffType.Defence:
