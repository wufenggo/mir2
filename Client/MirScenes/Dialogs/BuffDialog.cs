...
namespace Client.MirScenes.Dialogs
{
    public sealed class BuffDialog : MirImageControl
    {
        ...
        private string CombinedBuffText()
        {
            ...
            if (buffHealth > 0)
                buffText += $"增加生命: {buffHealth}";

            if (buffMana > 0)
                buffText += $"增加魔力: {buffMana}";

            if (buffBagWeight > 0)
                buffText += $"增加背包重量: {buffBagWeight}";
